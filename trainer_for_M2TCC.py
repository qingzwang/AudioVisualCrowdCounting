...
class Trainer():
    def __init__(self, dataloader, cfg, cfg_data, pwd):
        ...
        if self.net_name in ['SANet', 'SANet_Audio']:
        ...
        self.train_loader, self.val_loader, self.test_loader, self.restore_transform = dataloader(cfg_data)
    ...
    def forward(self):
        ...
        for epoch in range(self.epoch, self.cfg.MAX_EPOCH):
            ...
            if epoch%self.cfg.VAL_FREQ==0 or epoch>self.cfg.VAL_DENSE_START:
                ...
                if self.data_mode in ['SHHA', 'SHHB', 'QNRF', 'UCF50', 'AC']:
                    self.validate_V1()
                    self.test_V1()
    ...
    def test_V1(self):  # validate_V1 for SHHA, SHHB, UCF-QNRF, UCF50, AC
